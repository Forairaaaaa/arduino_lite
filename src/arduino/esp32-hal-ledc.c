...
bool ledcAttachChannel(uint8_t pin, uint32_t freq, uint8_t resolution, uint8_t channel)
{
    ...
    ledc_timer_config_t ledc_timer = {
        .speed_mode = group, .timer_num = timer, .duty_resolution = resolution, .freq_hz = freq, .clk_cfg = LEDC_DEFAULT_CLK};
    ...
    ledc_channel_config_t ledc_channel = {.speed_mode = group,
                                          .channel = (channel % 8),
                                          .timer_sel = timer,
...
uint32_t ledcWriteTone(uint8_t pin, uint32_t freq)
{
    ...
    if (bus != NULL)
    {
        ...
        ledc_timer_config_t ledc_timer = {
            .speed_mode = group, .timer_num = timer, .duty_resolution = 10, .freq_hz = freq, .clk_cfg = LEDC_DEFAULT_CLK};
...
uint32_t ledcChangeFrequency(uint8_t pin, uint32_t freq, uint8_t resolution)
{
    ...
    if (bus != NULL)
    {
        ...
        ledc_timer_config_t ledc_timer = {.speed_mode = group,
                                          .timer_num = timer,
...
bool ledcOutputInvert(uint8_t pin, bool out_invert)
{
    ...
    if (bus != NULL)
    {
        ...
        esp_rom_gpio_connect_out_signal(
            pin, ((bus->channel / 8 == 0) ? LEDC_HS_SIG_OUT0_IDX : LEDC_LS_SIG_OUT0_IDX) + ((bus->channel) % 8), out_invert, 0);
#else
...
static bool
ledcFadeConfig(uint8_t pin, uint32_t start_duty, uint32_t target_duty, int max_fade_time_ms, void (*userFunc)(void*), void* arg)
{
    ledc_channel_handle_t* bus = (ledc_channel_handle_t*)perimanGetPinBus(pin, ESP32_BUS_TYPE_LEDC);
    if (bus != NULL)
    {
...
bool ledcFadeWithInterruptArg(
    uint8_t pin, uint32_t start_duty, uint32_t target_duty, int max_fade_time_ms, void (*userFunc)(void*), void* arg)
{
